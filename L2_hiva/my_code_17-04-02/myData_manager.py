...
class DataManager(data_manager.DataManager):
    ...
    def __init__(self, predictedLabel, label):
        ''' New contructor.'''
        data_manager.DataManager.__init__(self, dataname, datadir)
    ...
    def afficheTableau(self): 
        ...
        nbErreurs = (nbErreurs * 100.0) / float(len(label))
        posAsPos = posAsPos * 100.0 / float(len(label))
        posAsNeg = posAsNeg * 100.0 / float(len(label))
        negAsNeg = negAsNeg * 100.0 / float(len(label))
        negAsPos = negAsPos * 100.0 / float(len(label))
